...
<body class="body">
                        ...
                        <div class="center-item">
                            <ul class="menu-list">
                                ...
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-layers"></i></div>
                                        <div class="text">Customer Managent</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                        ...
                                        <li class="sub-menu-item">
                                            <a href="{{ route('admin.locations') }}" class="">
                                ...
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-layers"></i></div>
                                        <div class="text">Landing page Management</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="{{ route('admin.campaigns') }}" class="">
                                                <div class="text"> Campaign List</div>
                                            </a>
                                        </li>
                                ...
                                <li class="menu-item has-children {{ Request::is('admin/orders*') || Request::is('admin/order-drafts*') ? 'active' : '' }}">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                    ...
                                    <ul class="sub-menu">
                                        ...
                                        <li class="sub-menu-item">
                                            <a href="{{ route('admin.order.drafts') }}" class="{{ Request::is('admin/order-drafts*') ? 'active' : '' }}">
                                                <div class="text">Order Drafts</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
    ...
    {{-- FilePond --}}
    <script src="{{ asset('admin-resource/lib/filepond/filepond-plugin-image-preview.min.js') }}"></script>
    <script src="{{ asset('admin-resource/lib/filepond/filepond-plugin-file-validate-type.min.js') }}"></script>
    <script src="{{ asset('admin-resource/lib/filepond/filepond-plugin-file-validate-size.min.js') }}"></script>
    <script src="{{ asset('admin-resource/lib/filepond/filepond.min.js') }}"></script>
    <script>
        ...
        /**
         * Create a FilePond instance on a file input.
         *
         * @param {string}  inputId   - The id of the <input type="file"> element
         * @param {object}  options   - FilePond options to merge/override defaults
         * @returns {FilePond}
         */
        window.createFilePond = function(inputId, options = {}) {
            const input = document.getElementById(inputId);
            if (!input) return null;

            // Capture name BEFORE FilePond.create() clears the attribute
            ...
            const pond = FilePond.create(input, Object.assign({}, defaults, options));

            // On submit: copy FilePond's in-memory files into a real file input so
            // the browser includes them in the multipart POST.
            // FilePond clears input.name during init, so we use the saved fieldName.
            if (form && fieldName) {
                form.addEventListener('submit', function() {
                    const files = pond.getFiles().map(f => f.file).filter(Boolean);
                    if (files.length === 0) return;

                    const dt = new DataTransfer();
                    files.forEach(f => dt.items.add(f));

                    // Find existing hidden file input or create a temporary one
                    let target = form.querySelector(`input[type="file"][name="${fieldName}"]`);
                    if (!target) {
