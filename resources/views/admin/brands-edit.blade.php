...
@section('content')
    @php
        $existingImageId = old('image_id', $brand->image_id);
        $existingPreview = $brand->media?->getThumbnailUrl() ?? $brand->media?->getUrl();
    @endphp
    ...
    <div class="main-content-inner">
        <div class="main-content-wrap">
            ...
            <div class="wg-box">
                <form class="form-new-product form-style-1" method="POST" action="{{ route('admin.brands.update') }}">
                    @csrf
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('name') is-invalid @enderror" type="text" placeholder="Brand name"
                            name="name" tabindex="0" aria-required="true" value="{{ old('name', $brand->name) }}" required
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('slug') is-invalid @enderror" type="text" placeholder="Brand Slug"
                            name="slug" tabindex="0" value="{{ old('slug', $brand->slug) }}" aria-required="true" required
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('sort_order') is-invalid @enderror"
                               ...
                               tabindex="0" value="{{ old('sort_order', $brand->sort_order) }}">
                        @error('sort_order')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                    </fieldset>
                    ...
                    <fieldset>
                        ...
                        <input type="hidden" name="image_id" id="brand_image_id" value="{{ $existingImageId }}">
                        ...
                        {{-- Preview --}}
                        <div id="brand_preview" style="{{ $existingPreview ? '' : 'display:none;' }} margin-bottom:12px;">
