...
@section('content')
    @php
        $existingImageId = old('image_id', $banner->image_id);
        $existingPreview = $banner->media?->getThumbnailUrl() ?? $banner->media?->getUrl();
    @endphp
                ...
                <form class="form-new-product form-style-1" method="POST"
                        ...
                        <div class="select flex-grow @error('zone') is-invalid @enderror">
                            <select name="zone" required>
                                @foreach ($zones as $key => $label)
                                    <option value="{{ $key }}" {{ old('zone', $banner->zone) === $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('zone')
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('title') is-invalid @enderror"
                               ...
                               tabindex="0" value="{{ old('title', $banner->title) }}">
                        @error('title')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                    </fieldset>
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('link') is-invalid @enderror"
                               ...
                               tabindex="0" value="{{ old('link', $banner->link) }}">
                        @error('link')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                    </fieldset>
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('sort_order') is-invalid @enderror"
                               ...
                               tabindex="0" value="{{ old('sort_order', $banner->sort_order) }}">
                        @error('sort_order')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                    </fieldset>
                    ...
                    <fieldset>
                        ...
                        <input type="hidden" name="image_id" id="banner_image_id" value="{{ $existingImageId }}">
                        ...
                        {{-- Preview --}}
                        <div id="banner_preview" style="{{ $existingPreview ? '' : 'display:none;' }} margin-bottom:12px;">
