...
@push('meta')
    @php
        ...
        $metaDesc =
            ...
            ($product->description ??
                'Buy ' . $metaName . ' from ' . $_sn . '. Premium quality products at the best price.');
...
@section('content')
    <div class="jtc-pd__shell" x-data="{
        qty: 1,
        activeMedia: 0,
        items: @js($fancyItems),
        inc() { this.qty += 1; },
        ...
        buyNow(productId, quantity) {
            const onUpdate = () => {
                window.location.href = @js(route('cart.checkout'));
        ...
        <div class="jtc-pd__crumb">
            ...
            @if ($firstCategory)
                <a href="{{ route('category.show', $firstCategory->slug) }}">{{ $firstCategory->name }}</a><span
                    class="sep">/</span>
            @endif
        ...
        <div class="jtc-pd__main">
            ...
            <div class="jtc-gallery">
                <div class="jtc-gallery__thumbs">
                    @foreach ($gallery as $i => $src)
                        <button type="button" class="jtc-gallery__thumb"
                            :class="activeMedia === {{ $i }} && 'is-active'"
                            @click="activeMedia = {{ $i }}" aria-label="View image">
                            <img src="{{ $src }}" alt="">
                        </button>
                    @endforeach
                    @if ($videoSrc)
                        <button type="button" class="jtc-gallery__thumb"
                            :class="activeMedia === {{ count($gallery) }} && 'is-active'"
                            @click="activeMedia = {{ count($gallery) }}" aria-label="View video">
                            <img src="{{ $gallery[0] ?? '' }}" alt="">
                            <span class="jtc-gallery__play"><svg viewBox="0 0 24 24" fill="currentColor" width="18"
                                    height="18">
                ...
                <div class="jtc-gallery__viewer" @click="openLightbox()">
                    @if ($videoSrc)
                        <template x-if="activeMedia === {{ count($gallery) }}">
                            <div class="jtc-gallery__videoposter">
                                ...
                                <span class="jtc-gallery__play jtc-gallery__play--lg"><svg viewBox="0 0 24 24"
                    ...
                    <div class="jtc-gallery__badges"
                        @if ($videoSrc) x-show="activeMedia !== {{ count($gallery) }}" @endif>
                        @if ($outOfStock)
            ...
            <div class="jtc-pd-info">
                ...
                <div class="jtc-pd-info__meta">
                    @if ($outOfStock)
                        <span class="jtc-pd-info__stock jtc-pd-info__stock--out">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" width="14"
                                height="14">
                    ...
                    <div class="jtc-pd-info__highlights">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                            @foreach ($product->sizes as $size)
                                <span class="jtc-badge"
                                    style="background:#fff;color:#14201c;border:1.5px solid #e6eae7">{{ $size->name }}</span>
                            @endforeach
                        </div>
                    </div>
                @endif
                ...
                <div class="jtc-qty">
                    <button type="button" @click="dec()" :disabled="{{ $outOfStock ? 'true' : 'false' }}"
                        aria-label="Decrease">−</button>
                    <span x-text="qty"></span>
                    <button type="button" @click="inc()" :disabled="{{ $outOfStock ? 'true' : 'false' }}"
                ...
                <div class="jtc-pd-actions__order">
                    <a href="https://wa.me/{{ $site['whatsapp'] ?? '8801329732724' }}?text={{ urlencode('Hi, I want to order: ' . $product->name) }}"
                        target="_blank" rel="noopener" class="jtc-pd-actions__wa">
                    ...
                    <a href="tel:+{{ $site['phone'] ?? '8801329732724' }}" class="jtc-pd-actions__call">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                            ...
                            <path
                                d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13 1 .37 1.94.72 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.87.35 1.81.59 2.81.72A2 2 0 0 1 22 16.92z">
                            </path>
                        </svg>
                        Call for order
                    </a>
                ...
                <div class="jtc-pd-info__reassure">
                    <div class="jtc-pd-info__reassure-item">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#1B7FC4" stroke-width="2" width="18"
                            height="18">
                            <path d="M3 7h13v8H3z"></path>
                            <path d="M16 10h3l2 2v3h-5z"></path>
                    ...
                    <div class="jtc-pd-info__reassure-item">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#1B7FC4" stroke-width="2" width="18"
                            height="18">
                            <path d="M3 9 12 4l9 5v8l-9 5-9-5z"></path>
